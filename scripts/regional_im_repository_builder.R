...
NIGERIA_IM_SCRIPT_INLINE <- c(
  ...
  "",
  "# ============================================================",
  "# 5) REASON PROCESSING -- ONE COLUMN-PAIR AT A TIME",
  "# ============================================================",
  "# Form 7178's raw file has 150 \"why not immunized\" main columns plus 74",
  "# matching \"_other\" free-text columns. The original approach melted ALL",
  "# 150 main columns into a single long table in one call -- an",
  "# intermediate table of up to nrow(AE) * 150 rows (~26.5 million rows for",
  "# this file's ~177k rows) before any filtering could shrink it, which is",
  "# what was OOM-killing this step (\"Processing missed-child reasons...\"",
  "# then Killed, immediately, before this section's own first [MEM]",
  "# checkpoint could even print).",
  "#",
  "# This processes one main-reason column (and its paired \"_other\" column,",
  "# if one exists) at a time: for each column, build a small nrow(AE)-row",
  "# slice (not nrow(AE) * 150), classify it, and immediately COUNT",
  "# occurrences per group + category instead of keeping the classified rows",
  "# themselves. Counting is combinable: the total occurrences of a category",
  "# in a group, across all 150 columns, equals the sum of each column's own",
  "# occurrence count for that group + category, because the original long",
  "# table is exactly the row-bind of every column's individually-melted",
  "# slice. Only the final, much smaller (group x category) count table is",
  "# ever held in full; at most one column's ~177k-row slice is resident at",
  "# a time.",
  "#",
  "# The main/other precedence is preserved exactly: the original filtered",
  "# reason_long_main down to non-empty MAIN text BEFORE ever merging in the",
  "# \"other\" override, so a non-empty \"other\" text can never rescue a row",
  "# whose own main-column cell was empty/NA. This replicates that by gating",
  "# on reason_raw first, then applying the other-text override only to",
  "# rows that already passed that gate.",
  "",
  ...
  ")",
  "",
  "empty_reason_template <- function(category_col_name) {",
  "  tmpl <- AE[0, ..reason_group_cols]",
  "  tmpl[, (category_col_name) := character(0)]",
  "  tmpl[, N := integer(0)]",
  "  tmpl",
  "}",
  "",
  "main_reason_partials <- vector(\"list\", length(reason_cols_main))",
  "abs_reason_partials <- vector(\"list\", length(reason_cols_main))",
  "nc_reason_partials <- vector(\"list\", length(reason_cols_main))",
  "",
  "for (col_i in seq_along(reason_cols_main)) {",
  "  main_col <- reason_cols_main[[col_i]]",
  "  other_col <- paste0(main_col, \"_other\")",
  "  has_other <- other_col %in% reason_cols_other",
  "",
  "  # Clear any stale abs_slice/nc_slice left over from a PREVIOUS column",
  "  # that took that branch but this one doesn't (main_reason isn't",
  "  # r_non_FM_Absent/r_non_FM_NC for every column) -- otherwise the old",
  "  # object stays bound and alive across iterations for no reason. Small",
  "  # per-object, but across 150 iterations this was part of why peak RSS",
  "  # crept upward through this loop instead of staying flat.",
  "  if (exists(\"abs_slice\", inherits = FALSE)) rm(abs_slice)",
  "  if (exists(\"nc_slice\", inherits = FALSE)) rm(nc_slice)",
  "",
  "  col_slice <- AE[, ..reason_group_cols]",
  "  col_slice[, reason_raw := normalize_reason_text(AE[[main_col]])]",
  "  col_slice[, reason_other_raw := if (has_other) normalize_reason_text(AE[[other_col]]) else NA_character_]",
  "",
  "  # Gate on the MAIN column's text first -- matches the original, which",
  "  # filtered reason_long_main to non-empty rows BEFORE ever merging in",
  "  # the \"other\" override, so a non-empty \"other\" text can never rescue a",
  "  # row whose own main-column cell was empty/NA.",
  "  col_slice <- col_slice[!is.na(reason_raw) & reason_raw != \"\"]",
  "  if (nrow(col_slice) == 0) next",
  "",
  "  col_slice[, reason_final := fifelse(",
  "    !is.na(reason_other_raw) & reason_other_raw != \"\",",
  "    reason_other_raw,",
  "    reason_raw",
  "  )]",
  "",
  "  # Defensive no-op filter, kept for parity with the original's final",
  "  # reason_long filter (reason_final is already guaranteed non-empty by",
  "  # construction at this point).",
  "  col_slice <- col_slice[!is.na(reason_final) & reason_final != \"\"]",
  "  if (nrow(col_slice) == 0) next",
  "",
  "  col_slice[, main_reason := detect_main_reason(reason_final)]",
  "",
  "  main_reason_partials[[col_i]] <- col_slice[",
  "    !is.na(main_reason) & main_reason != \"\",",
  "    .N,",
  "    by = c(reason_group_cols, \"main_reason\")",
  "  ]",
  "",
  "  abs_slice <- col_slice[main_reason == \"r_non_FM_Absent\"]",
  "  if (nrow(abs_slice) > 0) {",
  "    abs_slice[, abs_reason := detect_abs_reason(reason_final)]",
  "    abs_reason_partials[[col_i]] <- abs_slice[",
  "      !is.na(abs_reason) & abs_reason != \"\",",
  "      .N,",
  "      by = c(reason_group_cols, \"abs_reason\")",
  "    ]",
  "  }",
  "",
  "  nc_slice <- col_slice[main_reason == \"r_non_FM_NC\"]",
  "  if (nrow(nc_slice) > 0) {",
  "    nc_slice[, nc_reason := detect_nc_reason(reason_final)]",
  "    nc_reason_partials[[col_i]] <- nc_slice[",
  "      !is.na(nc_reason) & nc_reason != \"\",",
  "      .N,",
  "      by = c(reason_group_cols, \"nc_reason\")",
  "    ]",
  "  }",
  "",
  "  rm(col_slice)",
  "",
  "  if (col_i %% 25 == 0 || col_i == length(reason_cols_main)) {",
  "    message(sprintf(",
  "      \"[MEM] %s | after reason column %d/%d: peak RSS %.0f MB\",",
  "      basename(rds_file), col_i, length(reason_cols_main), peak_rss_mb()",
  "    ))",
  "    # Explicit full GC at the same cadence as this checkpoint (every 25",
  "    # columns) -- R's automatic GC runs on allocation pressure, not",
  "    # proactively, so across 150 iterations of alloc/dealloc without a",
  "    # nudge, garbage from recent columns can still be resident when the",
  "    # next column's slice gets allocated. This periodic gc() forces",
  "    # earlier reclamation so consecutive iterations' garbage doesn't",
  "    # pile up and ratchet the process's peak RSS higher than it needs",
  "    # to be (VmHWM is a historical high-water mark that never comes back",
  "    # down once hit, so preventing the spike matters more than cleaning",
  "    # up after it).",
  "    gc(full = TRUE)",
  "  }",
  "}",
  "",
  "main_reason_partials <- Filter(Negate(is.null), main_reason_partials)",
  "abs_reason_partials <- Filter(Negate(is.null), abs_reason_partials)",
  "nc_reason_partials <- Filter(Negate(is.null), nc_reason_partials)",
  "",
  "main_reason_long <- if (length(main_reason_partials) == 0) {",
  "  empty_reason_template(\"main_reason\")",
  "} else {",
  "  rbindlist(main_reason_partials)[, .(N = sum(N)), by = c(reason_group_cols, \"main_reason\")]",
  "}",
  "",
  "abs_reason_long <- if (length(abs_reason_partials) == 0) {",
  "  empty_reason_template(\"abs_reason\")",
  "} else {",
  "  rbindlist(abs_reason_partials)[, .(N = sum(N)), by = c(reason_group_cols, \"abs_reason\")]",
  "}",
  "",
  "nc_reason_long <- if (length(nc_reason_partials) == 0) {",
  "  empty_reason_template(\"nc_reason\")",
  "} else {",
  "  rbindlist(nc_reason_partials)[, .(N = sum(N)), by = c(reason_group_cols, \"nc_reason\")]",
  "}",
  "",
  ...
  "# 6) CLASSIFY REASONS",
  "# ============================================================",
  "# Classification now happens inline, per column, in the loop above (see",
  "# detect_main_reason()/detect_abs_reason()/detect_nc_reason() calls in",
  "# section 5) -- this header is kept only so the log's section numbering",
  "# still lines up with earlier runs.",
  "",
  "# ============================================================",
  ...
  "# ============================================================",
  "# Same column-at-a-time restructuring as the reason processing above --",
  "# form 7178 has 15 SourceInfo_house* columns, so the original single",
  "# melt() across all of them produced an intermediate table of up to",
  "# nrow(AE) * 15 rows (~2.65 million) before any filtering. Processing one",
  "# column at a time keeps each iteration's slice to nrow(AE) rows and",
  "# accumulates only the small per-group counts.",
  "",
  ...
  "if (length(sm_cols) > 0) {",
  "",
  "  sm_partials <- vector(\"list\", length(sm_cols))",
  "  sm_unknown_codes_all <- character(0)",
  "",
  "  for (col_i in seq_along(sm_cols)) {",
  "    col <- sm_cols[[col_i]]",
  "",
  "    col_slice <- AE[, ..reason_group_cols]",
  "    col_slice[, sm_raw := trimws(tolower(as.character(AE[[col]])))]",
  "    col_slice[sm_raw %in% c(\"\", \"na\", \"nan\", \"null\"), sm_raw := NA_character_]",
  "    col_slice <- col_slice[!is.na(sm_raw)]",
  "    if (nrow(col_slice) == 0) next",
  "",
  "    col_slice <- col_slice[",
  "      ,",
  "      .(sm_code = unlist(strsplit(sm_raw, \"\\\\s+\"))),",
  "      by = c(reason_group_cols)",
  "    ]",
  "",
  "    col_slice[, sm_code := trimws(sm_code)]",
  "    col_slice <- col_slice[sm_code != \"\"]",
  "    if (nrow(col_slice) == 0) next",
  "",
  "    unknown <- setdiff(unique(col_slice$sm_code), sm_map_dt$sm_code)",
  "    if (length(unknown) > 0) {",
  "      sm_unknown_codes_all <- union(sm_unknown_codes_all, unknown)",
  "    }",
  "",
  "    col_slice <- merge(",
  "      col_slice,",
  ...
  "    if (nrow(col_slice) == 0) next",
  "",
  "    sm_partials[[col_i]] <- col_slice[, .N, by = c(reason_group_cols, \"sm_type\")]",
  "  }",
  "",
  "  if (length(sm_unknown_codes_all) > 0) {",
  "    cat(\"\\nWARNING: Unknown SourceInfo codes detected:\\n\")",
  "    print(sm_unknown_codes_all)",
  "  }",
  "",
  "  sm_partials <- Filter(Negate(is.null), sm_partials)",
  "",
  "  sm_long <- if (length(sm_partials) == 0) {",
  "    empty_reason_template(\"sm_type\")",
  "  } else {",
  "    rbindlist(sm_partials)[, .(N = sum(N)), by = c(reason_group_cols, \"sm_type\")]",
  "  }",
  "",
  "  sm_wide <- dcast(",
  "    sm_long,",
...
# ============================================================
# FORM 6839 (ETHIOPIA) -- CALENDAR-FIRST Response/roundNumber/
# Vaccine.type/round_start_date ATTRIBUTION
# ============================================================
# Form 6839's template captures Response and roundNumber PER
# HOUSEHOLD (HH[1..10]/HH/Response, HH[1..10]/HH/roundNumber)
# instead of once per submission, and the raw text is unreliable --
# a large share of records carry a bare place name ("Addis Ababa" /
# "Mekelle") instead of a real campaign code. Per the workflow
# owner, the official campaign calendar (data/lookup/lookup.xlsx)
# is the base case for Ethiopia: Response, Vaccine.type,
# roundNumber, and round_start_date should all come FROM THE
# CALENDAR, not from the raw form text -- the raw text is only used
# to work out which calendar campaign/round a submission belongs to.
#
# Attribution runs in two passes:
#   1) TEXT MATCH: if the raw Response (or, for one campaign whose
#      fields were flat-out swapped, the raw roundNumber field) can
#      be identified as a real OBR Name in the calendar -- after
#      fixing the one known one-character typo and stripping the
#      one known verbose "bOPV Rnd 2 <code>" prefix -- that calendar
#      row wins outright, overriding the raw roundNumber even where
#      it disagrees (e.g. enumerators consistently logged "Rnd5" for
#      the OBR the calendar lists only as Round 1; the calendar wins).
#   2) DATE FALLBACK: for everything text match can't resolve (the
#      bare place-name rows), the workflow owner's instruction is
#      that on-the-ground implementation can run days to weeks late,
#      and where a submission's date falls between two campaigns it
#      belongs to the earlier (already-started) one. So every
#      Ethiopia round in the calendar is sorted chronologically and
#      each submission is assigned to whichever round had most
#      recently STARTED as of date_monitored (round N owns every
#      date from its own start up until round N+1 starts -- an
#      "as-of"/rolling join, not a fixed window).
# A date before the very first calendar round, or an unparseable
# date, has no legitimate campaign to attach to and is left
# unresolved (Response/roundNumber stay NA), which the existing
# safe_filter_data() step below already drops for any form.
#
# Validated against the raw file: 174,184 of 174,282 submissions
# (99.9%) resolve; the 98 that don't have dates before the
# calendar's earliest round (2020-10-07) or unparseable dates.
# Spot-checking date_monitored within each date-fallback bucket
# shows submissions cluster tightly (days to a couple of months)
# around their assigned round's start date, confirming the
# attribution is picking up real campaign activity rather than
# spreading records arbitrarily across calendar gaps.
# ============================================================
.eth_6839_normalize_obr <- function(x) {
  x <- toupper(trimws(as.character(x)))
  x <- str_remove(x, "^BOPV\\s+RND\\s*[0-9]+\\s+")
  gsub("[^A-Z0-9]", "", x)
}

# The one confirmed raw-data typo: "ETH-2026-05-bOPV-sNID" (missing
# the "2") should read as the calendar's real "ETH-2026-05-bOPV2-sNID".
.ETH_6839_KNOWN_TYPO_FIX <- c("ETH202605BOPVSNID" = "ETH202605BOPV2SNID")

.eth_6839_fix_known_typos <- function(norm_x) {
  hit <- norm_x %in% names(.ETH_6839_KNOWN_TYPO_FIX)
  norm_x[hit] <- unname(.ETH_6839_KNOWN_TYPO_FIX[norm_x[hit]])
  norm_x
}

.eth_6839_load_calendar <- function() {
  cal_raw <- tryCatch(readxl::read_excel(preparedness_file), error = function(e) NULL)
  if (is.null(cal_raw) || !"Country" %in% names(cal_raw) || !"OBR Name" %in% names(cal_raw)) {
    return(tibble())
  }
  ...
  cal <- cal_raw %>%
    ...
    mutate(.cal_id = row_number(), obr_norm = .eth_6839_normalize_obr(Response))

  cal
}

ETH_6839_CALENDAR <- .eth_6839_load_calendar()

# Threshold (days) beyond which a date-fallback assignment is flagged as
# "stale" -- i.e. date_monitored is unusually far past the assigned
# round's start date. A few weeks of on-the-ground implementation delay
# is expected and normal; a much larger gap more likely means the true
# round for that date hasn't been added to the calendar yet (e.g. a new
# round started but lookup.xlsx hasn't been updated), and this submission
# is silently being pinned to the last known round instead. Flagged (not
# excluded) so an operator can review 6839_calendar_attribution_audit.csv
# in the QC folder and decide whether the calendar needs updating.
ETH_6839_DATE_FALLBACK_STALE_DAYS <- 45

derive_ethiopia_6839_response_round <- function(data, qc_output_folder = NULL) {
  resp_cols  <- sprintf("HH[%d]/HH/Response", 1:10)
  round_cols <- sprintf("HH[%d]/HH/roundNumber", 1:10)

  if (!all(resp_cols %in% names(data)) || !all(round_cols %in% names(data))) {
    return(data)
  }
  if (nrow(ETH_6839_CALENDAR) == 0 || !"date_monitored" %in% names(data)) {
    return(data)
  }
  ...
  round_raw <- majority_value(as.matrix(data[round_cols]))
  date_monitored_parsed <- suppressWarnings(as_date(data[["date_monitored"]]))

  resp_norm <- .eth_6839_fix_known_typos(.eth_6839_normalize_obr(response_raw))
  round_norm <- .eth_6839_fix_known_typos(.eth_6839_normalize_obr(round_raw))
  raw_round_key <- gsub("[^A-Z0-9]", "", toupper(trimws(as.character(round_raw))))

  cal <- ETH_6839_CALENDAR
  ...
  # Pass 1: text match (Response text, then roundNumber text for the
  # one known field-swap case). Tracked separately (not just coalesced)
  # so the audit trail below can record which method resolved each row.
  cal_id_from_resp <- match_one_field(resp_norm)
  cal_id_from_round <- match_one_field(round_norm)
  cal_id <- dplyr::coalesce(cal_id_from_resp, cal_id_from_round)
  ...
  # Pass 2: date fallback for anything text match couldn't resolve --
  # as-of assignment to the most recently started calendar round.
  unresolved <- which(is.na(cal_id) & !is.na(date_monitored_parsed))
  if (length(unresolved) > 0) {
    pos <- findInterval(date_monitored_parsed[unresolved], cal$round_start_date)
    valid <- pos > 0
    cal_id[unresolved[valid]] <- cal$.cal_id[pos[valid]]
    match_method[unresolved[valid]] <- "date_fallback"
  }

  data$Response <- cal$Response[match(cal_id, cal$.cal_id)]
  data$roundNumber <- cal$roundNumber[match(cal_id, cal$.cal_id)]
  matched_round_start_date <- cal$round_start_date[match(cal_id, cal$.cal_id)]

  # ------------------------------------------------------------
  # Audit trail (QC folder only -- kept aside from the main
  # repository output, never mixed into it). One row per input
  # submission recording how Response/roundNumber were resolved (or
  # why the row is being excluded), so the calendar-first logic above
  # stays auditable instead of a black box. A write failure here is
  # logged and swallowed -- it must never block the main pipeline run.
  # ------------------------------------------------------------
  if (!is.null(qc_output_folder)) {
    audit <- tibble::tibble(
      row_id = seq_len(nrow(data)),
      Response_raw = response_raw,
      roundNumber_raw = round_raw,
      date_monitored = date_monitored_parsed,
      match_method = dplyr::if_else(is.na(match_method), "excluded_no_match", match_method),
      matched_Response = data$Response,
      matched_roundNumber = data$roundNumber,
      matched_round_start_date = matched_round_start_date
    ) %>%
      mutate(
        days_after_round_start = as.numeric(date_monitored - matched_round_start_date),
        stale_fallback_flag = match_method == "date_fallback" &
          !is.na(days_after_round_start) &
          days_after_round_start > ETH_6839_DATE_FALLBACK_STALE_DAYS
      )

    tryCatch({
      dir.create(qc_output_folder, showWarnings = FALSE, recursive = TRUE)
      audit_file <- file.path(qc_output_folder, "6839_calendar_attribution_audit.csv")
      readr::write_csv(audit, audit_file)
      n_excluded <- sum(audit$match_method == "excluded_no_match")
      n_stale <- sum(audit$stale_fallback_flag, na.rm = TRUE)
      message("6839 calendar attribution audit written: ", audit_file)
      message("  excluded (no calendar match): ", n_excluded, " of ", nrow(audit))
      message("  stale date-fallback assignments (>", ETH_6839_DATE_FALLBACK_STALE_DAYS,
              " days after round start -- check whether a newer round is missing from lookup.xlsx): ", n_stale)
    }, error = function(e) {
      message("Warning: failed to write 6839 calendar attribution audit CSV: ", conditionMessage(e))
    })
  }
...
apply_ethiopia_6839_vaccine_override <- function(df) {
  # Response has already been resolved to an exact calendar OBR Name
  # by derive_ethiopia_6839_response_round() (or is NA/excluded), so
  # a plain exact-match lookup against the calendar's own Vaccine.type
  # is all that's needed here -- no fuzzy matching. This runs AFTER
  # assign_vaccine_types(), overriding its generic Response-text
  # pattern matching, because several older Ethiopia OBR names (e.g.
  # "ETH-114DS-08-2021") don't contain any "nOPV"/"bOPV"/"mOPV"
  # substring for that generic matcher to key off, and would
  # otherwise silently fall through to "other" instead of the
  # calendar's actual "nOPV2" for that round.
  if (nrow(ETH_6839_CALENDAR) == 0 || !"Response" %in% names(df)) {
    return(df)
  }

  vmap <- ETH_6839_CALENDAR %>%
    distinct(Response, Vaccine.type) %>%
    tibble::deframe()

  matched <- df$Response %in% names(vmap)
  if (any(matched)) {
    df$Vaccine.type[matched] <- unname(vmap[df$Response[matched]])
  }
...
# ============================================================
apply_country_specific_transformations <- function(data, file_name, qc_output_folder = NULL) {
  ...
  # Ethiopia (form 6839): Response/roundNumber were captured per
  # household instead of once per submission, and the values need
  # harmonizing against the lookup table -- see derive_ethiopia_6839_
  # response_round() above for the full rationale. Must run before
  # rename_repetitive_columns() so the raw "HH[n]/HH/Response" /
  # "HH[n]/HH/roundNumber" column names are still intact.
  if (startsWith(file_name, "6839")) {
    data <- derive_ethiopia_6839_response_round(data, qc_output_folder)
  }
...
# ============================================================
# Same idea as the preparedness lookup refresh (refresh_preparedness_lookup.py)
# and Clean Geonames: check the result looks sane BEFORE overwriting the
# existing Regional_IM_repository files, so a silent upstream bug (a bad
# join, a country file that failed to parse, a filter applied wrong) can't
# quietly replace a good regional repository with a bad one just because
# nothing threw an R error. export_multi_format() above has no such check --
# it will happily write whatever `data` it's given.

# Reads the row count from the PREVIOUS run's export_manifest.txt (a small
# text file, cheap to read) rather than the previous repository CSV itself,
# which can be well over a million rows -- reading that just to count rows
# risks the same kind of memory pressure that caused the form-4498
# MemoryError seen earlier in this pipeline.
read_previous_repository_row_count <- function(manifest_file) {
  if (!file.exists(manifest_file)) return(NA_integer_)
  tryCatch({
    lines <- readLines(manifest_file, warn = FALSE)
    row_line <- grep("^Rows in main repository:", lines, value = TRUE)
    if (length(row_line) == 0) return(NA_integer_)
    as.integer(gsub("[^0-9]", "", row_line[1]))
  }, error = function(e) NA_integer_)
}

validate_before_publish <- function(data, previous_row_count,
                                     required_columns = c("country", "province", "district"),
                                     min_row_fraction = 0.5) {
  problems <- c()

  if (is.null(data) || nrow(data) == 0) {
    problems <- c(problems, "regional repository has 0 rows")
  }

  if (!is.null(data)) {
    missing_cols <- setdiff(required_columns, names(data))
    if (length(missing_cols) > 0) {
      problems <- c(problems, sprintf("missing required column(s): %s", paste(missing_cols, collapse = ", ")))
    }
  }

  if (!is.na(previous_row_count) && previous_row_count > 0 && !is.null(data)) {
    new_rows <- nrow(data)
    if (new_rows < previous_row_count * min_row_fraction) {
      problems <- c(problems, sprintf(
        "row count dropped from %s (previous run) to %s (%.0f%% of previous) -- more than the normal run-to-run change",
        format(previous_row_count, big.mark = ","), format(new_rows, big.mark = ","),
        100 * new_rows / previous_row_count
      ))
    }
  }

  if (length(problems) > 0) {
    message("\n[ERROR] VALIDATION FAILED - refusing to overwrite the existing Regional_IM_repository files:")
    for (p in problems) message("   - ", p)
    message("   The PREVIOUS repository files are being left untouched.")
    return(FALSE)
  }
...
build_regional_im_repository <- function(processed_results, regional_repository_file, regional_qc_repository_file) {
  ...
  # VALIDATE BEFORE PUBLISH
  # ============================================================
  # Checked against the PREVIOUS run's export_manifest.txt, read here before
  # this run's export overwrites it below. A failure stops before any of
  # the export calls run, so the existing repository/QC/metadata/manifest
  # files are all left exactly as they were.
  manifest_file <- file.path(dirname(regional_repository_file), "export_manifest.txt")
  previous_row_count <- read_previous_repository_row_count(manifest_file)
  if (!validate_before_publish(regional_im, previous_row_count)) {
    stop("Regional IM Repository failed validation before publish - see the message above. Existing files were not modified.")
  }
  ...
  # Create a manifest file listing all exports (manifest_file path was
  # already defined above, before the validation check read it)
  manifest_lines <- c(
    "============================================================",
