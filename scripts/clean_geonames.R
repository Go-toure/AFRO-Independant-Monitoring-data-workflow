#!/usr/bin/env Rscript
# ============================================================
# AFRO Independent Monitoring (IM) Geonames Cleaning Pipeline
# Complete Version
#
# Purpose:
# - Read the regional IM repository generated by regional_im_repository_builder.R
# - Standardize country, province, and district names
# - Preserve the complete original geonames correction mappings
# - Harmonize dates with the lookup file when available
# - Export cleaned IM repository as CSV, RDS, and Parquet
#
# Central workflow folder:
#   C:/Users/TOURE/Documents/im_workflow
#
# INPUT:
#   data/final/Regional_IM_repository.csv (or .rds, .parquet)
#
# OUTPUT:
#   data/final/Regional_IM_repository_cleaned.csv
#   data/final/Regional_IM_repository_cleaned.rds
#   data/final/Regional_IM_repository_cleaned.parquet
# ============================================================
...
INPUT_FILE <- file.path(FINAL_DIR, "Regional_IM_repository.csv")
LOOKUP_FILE <- file.path(LOOKUP_DIR, "lookup.xlsx")

OUTPUT_FILE_CSV <- file.path(FINAL_DIR, "Regional_IM_repository_cleaned.csv")
OUTPUT_FILE_RDS <- file.path(FINAL_DIR, "Regional_IM_repository_cleaned.rds")
OUTPUT_FILE_PARQUET <- file.path(FINAL_DIR, "Regional_IM_repository_cleaned.parquet")
SUMMARY_FILE <- file.path(FINAL_DIR, "IM_geonames_cleaning_summary.csv")
...
log_appender(appender_file(file.path(LOGS_DIR, "im_geonames_clean.log")))
log_info("============================================================")
log_info("Starting AFRO IM Geonames Cleaning Pipeline")
log_info("============================================================")
log_info("Input file: {INPUT_FILE}")
log_info("Lookup file: {LOOKUP_FILE}")
log_info("Output CSV: {OUTPUT_FILE_CSV}")
log_info("Output RDS: {OUTPUT_FILE_RDS}")
log_info("Output Parquet: {OUTPUT_FILE_PARQUET}")
...
normalize_admin_text <- function(x) {
  x <- as.character(x)
  x <- trimws(x)
  x[x %in% c("", "NA", "NaN", "NULL", "null", "nan")] <- NA_character_
  x <- toupper(x)
...
read_input_data <- function(input_file) {
  ...
  if (!file.exists(input_file)) {
    alt_parquet <- sub("\\.csv$", ".parquet", input_file, ignore.case = TRUE)
    alt_csv <- sub("\\.parquet$", ".csv", input_file, ignore.case = TRUE)
    alt_rds <- sub("\\.csv$", ".rds", input_file, ignore.case = TRUE)
    
    if (file.exists(alt_parquet)) {
      log_info("CSV not found. Found Parquet fallback: {alt_parquet}")
      input_file <- alt_parquet
    } else if (file.exists(alt_rds)) {
      log_info("CSV not found. Found RDS fallback: {alt_rds}")
      input_file <- alt_rds
    } else if (file.exists(alt_csv)) {
      log_info("Parquet not found. Found CSV fallback: {alt_csv}")
      input_file <- alt_csv
    } else {
  ...
  if (grepl("\\.parquet$", input_file, ignore.case = TRUE)) {
    ...
    log_info("Read RDS file: {nrow(data)} rows, {ncol(data)} columns")
  } else {
...
# ============================================================
# STANDARDIZE IM COLUMN NAMES
# ============================================================

standardize_im_columns <- function(data) {
  log_info("Standardizing IM repository column names...")
  
  data <- data %>%
    rename_with(~ stringr::str_replace_all(.x, "\\s+", "_"))
  
  # Regional builder may output Country/Region/District/Vaccine.type.
  # Geonames functions below expect lower-case country/province/district.
  rename_pairs <- list(
    country = c("country", "Country", "COUNTRY"),
    province = c("province", "Province", "PROVINCE", "Region", "REGION", "region"),
    district = c("district", "District", "DISTRICT"),
    response = c("response", "Response", "RESPONSE"),
    roundNumber = c("roundNumber", "RoundNumber", "round_number", "Round_Number"),
    `vaccine.type` = c("vaccine.type", "Vaccine.type", "Vaccine_type", "vaccine_type", "Vaccine_Type")
  )
  
  for (target in names(rename_pairs)) {
    if (!target %in% names(data)) {
      old <- intersect(rename_pairs[[target]], names(data))
      if (length(old) > 0) {
        data <- data %>% rename(!!target := all_of(old[1]))
      }
    }
  }
  
  required_cols <- c("country", "province", "district", "response", "roundNumber")
  missing_cols <- setdiff(required_cols, names(data))
  if (length(missing_cols) > 0) {
    stop("Missing required columns after standardization: ", paste(missing_cols, collapse = ", "))
  }
  
  if (!"vaccine.type" %in% names(data)) {
    data$`vaccine.type` <- NA_character_
  }
  
  data <- data %>%
    mutate(
      country = normalize_admin_text(country),
      province = normalize_admin_text(province),
      district = normalize_admin_text(district),
      response = as.character(response),
      roundNumber = normalize_admin_text(roundNumber),
      `vaccine.type` = as.character(`vaccine.type`)
    )
...
harmonize_dates <- function(data, lookup_file) {
  log_info("Harmonizing IM dates with lookup table...")
  
  if (!file.exists(lookup_file)) {
    log_warn("Lookup file not found: {lookup_file}")
    log_info("Dates will be kept as-is from the IM repository")
    return(data)
  }
  
  date_lookup <- readxl::read_excel(lookup_file)
  log_info("Read lookup file from {lookup_file}")
  log_info("Lookup contains {nrow(date_lookup)} rows")
  
  required_lookup_cols <- c("OBR Name", "Vaccines", "Round Number", "Round Start Date")
  missing_lookup_cols <- setdiff(required_lookup_cols, names(date_lookup))
  
  if (length(missing_lookup_cols) > 0) {
    log_warn("Lookup file is missing expected columns: {paste(missing_lookup_cols, collapse = ', ')}")
    log_info("Dates will be kept as-is from the IM repository")
    return(data)
  }
  ...
  lookup_data <- date_lookup %>%
    mutate(
      `Round Number` = case_when(
        ...
        TRUE ~ as.character(`Round Number`)
      )
    ) %>%
    transmute(
      response = as.character(`OBR Name`),
      `vaccine.type` = as.character(Vaccines),
      roundNumber = normalize_admin_text(`Round Number`),
      lookup_round_start_date = safe_as_date(`Round Start Date`),
      lookup_start_date_IM_end = lookup_round_start_date + 4,
      lookup_end_date_IM_end = lookup_start_date_IM_end + 1
    )
  
  log_info("Lookup table prepared with {nrow(lookup_data)} entries")
  
  result <- data %>%
    left_join(lookup_data, by = c("response", "vaccine.type", "roundNumber"))
  
  if ("round_start_date" %in% names(result)) {
    result <- result %>%
      mutate(round_start_date = coalesce(lookup_round_start_date, safe_as_date(round_start_date)))
  } else {
    result <- result %>%
      mutate(round_start_date = lookup_round_start_date)
  }
  
  if ("start_date_IM_end" %in% names(result)) {
    result <- result %>%
      mutate(start_date_IM_end = coalesce(lookup_start_date_IM_end, safe_as_date(start_date_IM_end)))
  } else if ("start_date" %in% names(result)) {
    result <- result %>%
      mutate(start_date_IM_end = coalesce(lookup_start_date_IM_end, safe_as_date(start_date)))
  } else {
    result <- result %>%
      mutate(start_date_IM_end = lookup_start_date_IM_end)
  }
  
  if ("end_date_IM_end" %in% names(result)) {
    result <- result %>%
      mutate(end_date_IM_end = coalesce(lookup_end_date_IM_end, safe_as_date(end_date_IM_end)))
  } else if ("end_date" %in% names(result)) {
    result <- result %>%
      mutate(end_date_IM_end = coalesce(lookup_end_date_IM_end, safe_as_date(end_date)))
  } else {
    result <- result %>%
      mutate(end_date_IM_end = lookup_end_date_IM_end)
  }
  
  result <- result %>%
    select(-any_of(c(
      "lookup_round_start_date",
      "lookup_start_date_IM_end",
      "lookup_end_date_IM_end"
    ))) %>%
...
# ============================================================
# FUNCTION: APPLY FINAL IM TRANSFORMATIONS
# ============================================================

apply_final_transformations <- function(data, start_date = START_DATE) {
  log_info("Applying final IM transformations...")
  
  date_cols <- intersect(
    c("round_start_date", "start_date_IM_end", "end_date_IM_end", "start_date", "end_date"),
    names(data)
  )
  
  for (dc in date_cols) {
    data[[dc]] <- safe_as_date(data[[dc]])
  }
  
  if ("start_date_IM_end" %in% names(data)) {
    data <- data %>%
      filter(is.na(start_date_IM_end) | start_date_IM_end >= as_date(start_date))
  } else if ("round_start_date" %in% names(data)) {
    data <- data %>%
      filter(is.na(round_start_date) | round_start_date >= as_date(start_date))
  }
  
  if ("round_start_date" %in% names(data)) {
    data <- data %>% mutate(year = year(round_start_date))
  }
  ...
  data <- data %>%
    ...
    distinct(im_unique_key, .keep_all = TRUE) %>%
    select(-im_unique_key)
  
  log_info("Final IM dataset: {nrow(data)} rows, {ncol(data)} columns")
  data
}

# ============================================================
# FUNCTION: STANDARDIZE OUTPUT COLUMN NAMES
# ============================================================

standardize_columns <- function(data) {
  log_info("Standardizing output column names...")
  ...
  if ("vaccine.type" %in% names(data)) {
    data <- data %>% rename(vaccine_type = `vaccine.type`)
  }
...
generate_summary_stats <- function(data) {
  log_info("Generating IM summary statistics...")
  
  date_var <- case_when(
    "start_date_IM_end" %in% names(data) ~ "start_date_IM_end",
    "round_start_date" %in% names(data) ~ "round_start_date",
    TRUE ~ NA_character_
  )
  
  date_range <- if (!is.na(date_var)) {
    paste(
      suppressWarnings(min(data[[date_var]], na.rm = TRUE)),
      "to",
      suppressWarnings(max(data[[date_var]], na.rm = TRUE))
    )
  } else {
    "Not available"
  }
  ...
  summary_stats <- data %>%
    summarise(
      ...
      total_u5_present = if ("u5_present" %in% names(data)) sum(u5_present, na.rm = TRUE) else NA_real_,
      total_u5_FM = if ("u5_FM" %in% names(data)) sum(u5_FM, na.rm = TRUE) else NA_real_,
      total_missed_child = if ("missed_child" %in% names(data)) sum(missed_child, na.rm = TRUE) else NA_real_,
      overall_cv = if (
        "u5_present" %in% names(data) &&
        "u5_FM" %in% names(data) &&
        sum(u5_present, na.rm = TRUE) > 0
      ) {
        round(sum(u5_FM, na.rm = TRUE) / sum(u5_present, na.rm = TRUE), 4)
      } else {
        NA_real_
      }
    )
  ...
  log_info("Overall IM CV: {summary_stats$overall_cv}")
  
  summary_stats
}

# ============================================================
# FUNCTION: WRITE OUTPUT FILES (CSV, RDS, and Parquet)
# ============================================================

# Retries a file-write operation when the target file is transiently locked
# (e.g. OneDrive/cloud sync mid-upload, an antivirus scan, or another app -
# such as the Shiny dashboard - holding it open for reading). Re-throws the
# original error after the final attempt so genuine failures still surface.
write_with_retry <- function(write_fn, label, max_attempts = 5, wait_seconds = 3) {
  for (attempt in seq_len(max_attempts)) {
    ...
    if (is.null(attempt_error)) return(invisible(TRUE))

    if (attempt == max_attempts) {
      log_error("Giving up writing {label} after {max_attempts} attempts: {conditionMessage(attempt_error)}")
      stop(attempt_error)
    }

    log_warn("Attempt {attempt}/{max_attempts} to write {label} failed ({conditionMessage(attempt_error)}); retrying in {wait_seconds}s...")
    Sys.sleep(wait_seconds)
  }
}

write_output_files <- function(data, output_csv = OUTPUT_FILE_CSV, output_rds = OUTPUT_FILE_RDS, output_parquet = OUTPUT_FILE_PARQUET) {
  log_info("Writing cleaned IM output files...")

  fs::dir_create(dirname(output_csv))

  # Write CSV (retried - see write_with_retry())
  write_with_retry(function() readr::write_csv(data, output_csv), basename(output_csv))
  csv_size <- round(file.size(output_csv) / 1024 / 1024, 2)
  log_info("Saved CSV to {output_csv} ({csv_size} MB)")

  # Write RDS (compressed, retried)
  write_with_retry(function() saveRDS(data, output_rds, compress = TRUE), basename(output_rds))
  rds_size <- round(file.size(output_rds) / 1024 / 1024, 2)
  log_info("Saved RDS to {output_rds} ({rds_size} MB)")

  # Write Parquet (retried)
  write_with_retry(function() arrow::write_parquet(data, output_parquet, compression = "snappy"), basename(output_parquet))
  parquet_size <- round(file.size(output_parquet) / 1024 / 1024, 2)
  log_info("Saved Parquet to {output_parquet} ({parquet_size} MB)")

  # Print summary to console
  cat("\n============================================================\n")
  cat("CLEANED IM REPOSITORY EXPORTED\n")
  cat("============================================================\n")
  cat(sprintf("  CSV:     %s (%.2f MB)\n", basename(output_csv), csv_size))
  cat(sprintf("  RDS:     %s (%.2f MB)\n", basename(output_rds), rds_size))
  cat(sprintf("  Parquet: %s (%.2f MB)\n", basename(output_parquet), parquet_size))
  cat("============================================================\n")
...
# ============================================================
# FUNCTION: VALIDATE BEFORE PUBLISH
# ============================================================
# Same idea as the preparedness lookup refresh (refresh_preparedness_lookup.py):
# check the result looks sane BEFORE overwriting the existing output files,
# so a silent bug upstream (a join that drops rows, a filter applied wrong,
# a lookup that stops matching) can't quietly replace a good cleaned file
# with a bad one just because this script itself didn't crash. Today,
# write_output_files() would happily overwrite Regional_IM_repository_cleaned.*
# with whatever `data` is, with zero checks on it.

# Reads the record count from the PREVIOUS run's summary file (a single-row
# CSV, cheap to read) rather than the previous cleaned CSV itself, which can
# be 1M+ rows -- reading that just to count rows would risk the same kind
# of memory pressure that caused the form-4498 MemoryError earlier.
read_previous_record_count <- function(summary_file) {
  if (!file.exists(summary_file)) return(NA_integer_)
  tryCatch({
    prev <- readr::read_csv(summary_file, show_col_types = FALSE, n_max = 1)
    if ("total_records" %in% names(prev)) as.integer(prev$total_records[1]) else NA_integer_
  }, error = function(e) NA_integer_)
}

validate_before_publish <- function(data, previous_record_count,
                                     required_columns = c("country", "province", "district"),
                                     min_row_fraction = 0.5) {
  problems <- c()

  if (is.null(data) || nrow(data) == 0) {
    problems <- c(problems, "cleaned dataset has 0 rows")
  }
  ...
  if (!is.na(previous_record_count) && previous_record_count > 0 && !is.null(data)) {
    new_rows <- nrow(data)
    if (new_rows < previous_record_count * min_row_fraction) {
      problems <- c(problems, sprintf(
        "row count dropped from %s (previous run) to %s (%.0f%% of previous) -- more than the normal run-to-run change",
        format(previous_record_count, big.mark = ","), format(new_rows, big.mark = ","),
        100 * new_rows / previous_record_count
      ))
    }
  }

  if (length(problems) > 0) {
    log_error("VALIDATION FAILED - refusing to overwrite the existing cleaned output files:")
    for (p in problems) log_error("  - {p}")
    cat("\n[ERROR] [Clean Geonames] Output failed validation - the PREVIOUS cleaned files are being kept untouched:\n")
    for (p in problems) cat(sprintf("   - %s\n", p))
    cat("   Check data/final/Regional_IM_repository.csv (this script's input) and the log above for what changed.\n")
    return(FALSE)
  }
...
run_cleaning_pipeline <- function(
    ...
    output_rds = OUTPUT_FILE_RDS,
    output_parquet = OUTPUT_FILE_PARQUET,
    start_date = START_DATE
) {
  
  log_info("============================================================")
  log_info("RUNNING IM GEONAMES CLEANING PIPELINE")
  log_info("============================================================")
  
  # STEP 1: Read input file
  log_info("STEP 1: Reading input file")
  ...
  # STEP 2: Standardize IM column names
  log_info("STEP 2: Standardizing IM column names")
  data <- standardize_im_columns(data)
  
  # STEP 3: Standardize country names
  log_info("STEP 3: Standardizing country names")
  data <- standardize_country_names(data)
  
  # STEP 4: Apply province mappings
  log_info("STEP 4: Applying province name corrections")
  data <- apply_province_mappings(data)
  
  # STEP 5: Apply district mappings
  log_info("STEP 5: Applying district name corrections")
  data <- apply_district_mappings(data)
  
  # STEP 6: Apply AFRO block classification
  log_info("STEP 6: Applying AFRO block classification")
  data <- apply_afro_block(data)
  
  # STEP 7: Harmonize dates with lookup table
  log_info("STEP 7: Harmonizing dates with lookup file")
  data <- harmonize_dates(data, lookup_file)
  
  # STEP 8: Apply final transformations
  log_info("STEP 8: Applying final transformations")
  data <- apply_final_transformations(data, start_date)
  
  # STEP 9: Standardize output column names
  log_info("STEP 9: Standardizing output column names")
  data <- standardize_columns(data)

  # STEP 9.5: Validate before publishing
  # previous_record_count is read BEFORE generate_summary_stats() (STEP 10)
  # overwrites the summary file it comes from, so it reflects the LAST
  # successful run, not this one. A failure here stops before STEP 10/11
  # touch anything, so the previous summary/CSV/RDS/Parquet files are all
  # left exactly as they were.
  previous_record_count <- read_previous_record_count(SUMMARY_FILE)
  if (!validate_before_publish(data, previous_record_count)) {
    return(NULL)
  }
  ...
  # STEP 11: Write output files (CSV, RDS, Parquet)
  write_output_files(data, output_csv, output_rds, output_parquet)
  
  log_info("============================================================")
  log_info("IM GEONAMES CLEANING PIPELINE COMPLETED SUCCESSFULLY")
  log_info("============================================================")
...
result <- tryCatch(
  run_cleaning_pipeline(INPUT_FILE, LOOKUP_FILE, OUTPUT_FILE_CSV, OUTPUT_FILE_RDS, OUTPUT_FILE_PARQUET, START_DATE),
  error = function(e) {
