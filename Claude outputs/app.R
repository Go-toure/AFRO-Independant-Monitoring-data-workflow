  ...
  output$sp_badge_ui <- renderUI({
    ...
    if (st == "ok")
      div(class="alert alert-success p-2 mt-2 small",
          bs_icon("check-circle-fill", style="vertical-align:-0.15em;margin-right:4px;"),
          "Upload complete")
    else
      div(class="alert alert-danger p-2 mt-2 small",
          bs_icon("x-circle-fill", style="vertical-align:-0.15em;margin-right:4px;"),
          "Error — check log")
  })
} # end server
shinyApp(ui, server)
